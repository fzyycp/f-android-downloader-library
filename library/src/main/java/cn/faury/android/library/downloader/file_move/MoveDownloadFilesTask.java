...
import cn.faury.android.library.downloader.listener.OnMoveDownloadFileListener;
import cn.faury.android.library.downloader.listener.OnMoveDownloadFilesListener;
...
class MoveDownloadFilesTask implements Runnable, Stoppable {
    ...
    private static final String TAG = MoveDownloadFilesTask.class.getName();
        ...
        try {
            ...
            for (int i = 0; i < mDownloadFilesNeedMove.size(); i++) {
                ...
                if (isStopped()) {
                    ...
                    Logger.w(TAG, ".run 批量移动任务被取消，无法继续移动，任务即将结束");
                ...
                if (mDownloadTaskPauseable.isDownloading(finalUrl)) {
                    ...
                    Logger.w(TAG, ".run 需要先暂停单个下载任务后移动，url:" + finalUrl);
                    ...
                    mDownloadTaskPauseable.pause(finalUrl, new OnStopFileDownloadTaskListener() {
                        ...
                        public void onStopFileDownloadTaskSucceed(String url) {
                            ...
                            Logger.d(TAG, TAG + ".run 暂停单个下载任务成功，开始移动，url:" + finalUrl);

                            // if the task stopped, notify completed
                            if (isStopped()) {

                                Logger.d(TAG, TAG + ".run 批量移动任务被取消，无法继续移动，任务即将结束");
                        ...
                        public void onStopFileDownloadTaskFailed(String url, StopDownloadFileTaskFailReason 
                            ...
                            Logger.d(TAG, TAG + ".run 暂停单个下载任务失败，无法移动，url:" + finalUrl);
            ...
            Logger.d(TAG, TAG + ".run 批量移动文件主任务【已结束】，但是通过暂停下载中的文件移动任务可能还没有结束");
        }
    }
    ...
    private class OnMoveSingleDownloadFileListener implements OnMoveDownloadFileListener {
        ...
        public void onMoveDownloadFileSuccess(DownloadFileInfo downloadFileMoved) {
            ...
            Logger.d(TAG, TAG + ".run 移动单个成功，已移动数量：" + mDownloadFilesMoved.size() + "，总共需要移动数量" +
    ...
    private void notifyMoveDownloadFilesPrepared() {
        ...
        Logger.d(TAG, TAG + ".run 准备批量移动，大小：" + mDownloadFilesNeedMove.size());
    ...
    private void notifyMovingDownloadFiles(DownloadFileInfo downloadFileInfo) {
        ...
        Logger.d(TAG, TAG + ".run 准备移动单个，url：" + url);
    ...
    private void notifyMoveDownloadFilesCompleted() {
        ...
        if (mIsNotifyFinish.compareAndSet(false, true)) {
            ...
            Logger.d(TAG, TAG + ".run，批量移动文件主任务和其它相关任务全部【已结束】，总共需要移动：" + mDownloadFilesNeedMove.size() + "，已移动：" +
    ...
    private void checkRollback() {
        if (CollectionsUtils.isEmpty(mDownloadFilesSkip)) {
            return;
        }
        ...
        for (DownloadFileInfo downloadFileInfo : mDownloadFilesSkip) {
            ...
            if (!FileUtils.isFilePath(oldDirPath) || oldDirPath.equals(downloadFileInfo.getFileDir())) {
                continue;
            }
