...
class DetectUrlFileTask implements Runnable {
    ...
    private static final String TAG = DetectUrlFileTask.class.getName();
    ...
    public void run() {
        ...
        try {
            ...
            Logger.d(TAG, ".run 探测文件，重定向：" + redirectCount + "次" + "，最大重定向次数：" + MAX_REDIRECT_TIMES +
                    "，url：" + mUrl);
            ...
            Logger.d(TAG, ".run Response Headers:" + HttpConnectionHelper.getStringHeaders(conn.getHeaderFields()));
            ...
            switch (conn.getResponseCode()) {
                ...
                case HttpURLConnection.HTTP_OK:
                    ...
                    if (TextUtils.isEmpty(fileName)) {
                        // get from url
                        fileName = UrlUtils.getFileNameByUrl(connectUrl, mCharset);
                    } else {
                        // check is decoded
                        if (UrlUtils.isEncoded(fileName, mCharset)) {
                            fileName = UrlUtils.decode(fileName, mCharset);
                        }
                    }
            ...
            Logger.d(TAG, ".run 探测文件任务【已结束】，是否有异常：" + hasException + "，是否强制探测模式：" + mIsForceDetect +
    ...
    private boolean notifyDetectNewDownloadFile(String fileName, String saveDir, long fileSize) {
        ...
        Logger.d(TAG, "探测文件不存在，需要创建，url：" + mUrl);
