package cn.faury.android.library.downloader.base;
...
public class BaseDownloadConfigBuilder {
    private static final String TAG = BaseDownloadConfigBuilder.class.getName();
    /**
    ...
    public BaseDownloadConfigBuilder configRetryDownloadTimes(int retryDownloadTimes) {
        if (retryDownloadTimes >= 0 && retryDownloadTimes <= MAX_RETRY_DOWNLOAD_TIMES) {
            ...
            Logger.w(TAG, "configRetryDownloadTimes 配置下载失败重试次数失败，retryDownloadTimes：" +
                    retryDownloadTimes);
        }
        return this;
    }
