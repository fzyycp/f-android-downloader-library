...
import cn.faury.android.library.downloader.file_download.base.DownloadTask;
import cn.faury.android.library.downloader.file_download.base.OnStopFileDownloadTaskListener;
import cn.faury.android.library.downloader.file_download.base.OnStopFileDownloadTaskListener.StopDownloadFileTaskFailReason;
import cn.faury.android.library.downloader.file_download.base.OnTaskRunFinishListener;
import cn.faury.android.library.downloader.file_download.file_saver.FileSaver;
import cn.faury.android.library.downloader.file_download.file_saver.FileSaver.FileSaveException;
import cn.faury.android.library.downloader.file_download.file_saver.FileSaver.OnFileSaveListener;
import cn.faury.android.library.downloader.file_download.http_downloader.ContentLengthInputStream;
import cn.faury.android.library.downloader.file_download.http_downloader.HttpDownloader;
import cn.faury.android.library.downloader.file_download.http_downloader.HttpDownloader.OnHttpDownloadListener;
import cn.faury.android.library.downloader.file_download.http_downloader.HttpDownloader.OnRangeChangeListener;
import cn.faury.android.library.downloader.file_download.http_downloader.Range;
import cn.faury.android.library.downloader.listener.OnFileDownloadStatusListener;
import cn.faury.android.library.downloader.listener.OnFileDownloadStatusListener.FileDownloadStatusFailReason;
import cn.faury.android.library.downloader.listener.OnFileDownloadStatusListener.OnFileDownloadStatusFailReason;
...
class DownloadTaskImpl implements DownloadTask, OnHttpDownloadListener, OnFileSaveListener, OnRangeChangeListener {
    ...
    private void init() {
        ...
        Logger.v(TAG, ".init 1、初始化新下载任务，url：" + getUrl());
    ...
    private boolean checkTaskCanExecute() {
        ...
        if (failReason == null && !UrlUtils.isUrl(url)) {
            // error, url illegal
            failReason = new OnFileDownloadStatusFailReason(url, "url illegal !", OnFileDownloadStatusFailReason
                    .TYPE_URL_ILLEGAL);
        }
        if (failReason == null && !FileUtils.isFilePath(mTaskParamInfo.getFilePath())) {
        ...
        if (failReason == null && (!FileUtils.canWrite(mTaskParamInfo.getTempFilePath()) || !FileUtils.canWrite
        ...
        if (failReason == null) {
            try {
                ...
                if (!FileUtils.isFilePath(checkPath)) {
                    ...
                    long freeSize = FileUtils.getAvailableSpace(checkPath);
    ...
    public void run() {
        ...
        try {
            ...
            // ------------start checking conditions------------
            {
                ...
                Logger.d(TAG, ".run 2、任务开始执行，正在获取资源，url：：" + url);
            ...
            Logger.d(TAG, ".run 7、文件下载任务【已结束】，是否有异常：" + hasException + "，url：" + url);
        }
    }
    ...
    public void onDownloadConnected(ContentLengthInputStream inputStream, long startPosInTotal) {
        ...
        Logger.d(TAG, ".run 3、已经连接到资源，url：" + getUrl());
    ...
    public void onSaveDataStart() {
        ...
        Logger.d(TAG, ".run 4、准备下载，url：" + getUrl());
    ...
    public void onSavingData(int increaseSize, long totalSize) {
        ...
        Logger.d(TAG, ".run 5、下载中，url：" + getUrl());
    ...
    private boolean notifyStatusWaiting() {
        try {
            ...
            Logger.d(TAG, "file-downloader-status 记录【等待状态】成功，url：" + getUrl());
    ...
    private boolean notifyStatusPreparing() {
        try {
            ...
            Logger.d(TAG, "file-downloader-status 记录【正在准备状态】成功，url：" + getUrl());
    ...
    private boolean notifyStatusPrepared() {
        try {
            ...
            Logger.d(TAG, "file-downloader-status 记录【已准备状态】成功，url：" + getUrl());
    ...
    private boolean notifyStatusDownloading(int increaseSize) {
        try {
            ...
            Logger.d(TAG, "file-downloader-status 记录【正在下载状态】成功，url：" + getUrl());
                ...
                try {
                        ...
                        case Status.DOWNLOAD_STATUS_PAUSED:
                            if (mOnFileDownloadStatusListener != null) {
                                if (mIsNotifyTaskFinish.compareAndSet(false, true)) {
                                    ...
                                    Logger.d(TAG, "file-downloader-status 记录【暂停状态】成功，url：" + getUrl());
                                }
                            }
                            break;
                        case Status.DOWNLOAD_STATUS_COMPLETED:
                            if (mOnFileDownloadStatusListener != null) {
                                if (mIsNotifyTaskFinish.compareAndSet(false, true)) {
                                    ...
                                    Logger.d(TAG, "file-downloader-status 记录【完成状态】成功，url：" + getUrl());
                                }
                            }
                            break;
                        case Status.DOWNLOAD_STATUS_ERROR:
                            if (mOnFileDownloadStatusListener != null) {
                                if (mIsNotifyTaskFinish.compareAndSet(false, true)) {
                                    ...
                                    Logger.d(TAG, "file-downloader-status 记录【错误状态】成功，url：" + getUrl());
                                }
                            }
                            break;
                        case Status.DOWNLOAD_STATUS_FILE_NOT_EXIST:
                            if (mOnFileDownloadStatusListener != null) {
                                if (mIsNotifyTaskFinish.compareAndSet(false, true)) {
                                    ...
                                    Logger.d(TAG, "file-downloader-status 记录【文件不存在状态】成功，url：" + getUrl());
                                }
                            }
                            break;
                    }
                    ...
                    if (mIsNotifyTaskFinish.compareAndSet(false, true)) {
                        ...
                        if (mOnFileDownloadStatusListener != null) {
                            ...
                            Logger.w(TAG, "file-downloader-status 记录【暂停/完成/出错状态】失败，url：" + getUrl());
                        }
                    }
                } finally {
    ...
    private void notifyStopTaskSucceedIfNecessary() {
        if (mOnStopFileDownloadTaskListener != null) {
            ...
            Logger.d(TAG, "file-downloader-status 通知【暂停任务】成功，url：" + getUrl());
        }
    }
    ...
    private void notifyStopTaskFailedIfNecessary(StopDownloadFileTaskFailReason failReason) {
        if (mOnStopFileDownloadTaskListener != null) {
            ...
            Logger.w(TAG, "file-downloader-status 通知【暂停任务】失败，url：" + getUrl());
        }
    }
    ...
    public void stop() {
        ...
        Logger.d(TAG, ".stop 结束任务执行，url：" + getUrl() + ",是否已经暂停：" + mIsTaskStop);
        ...
        if (Thread.currentThread() == mCurrentTaskThread) {
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    Logger.d(TAG, ".stop 结束任务执行(主线程发起)，url：" + getUrl() + ",是否已经暂停：" + mIsTaskStop);
            ...
            Logger.d(TAG, ".stop 结束任务执行(其它线程发起)，url：" + getUrl() + ",是否已经暂停：" + mIsTaskStop);
