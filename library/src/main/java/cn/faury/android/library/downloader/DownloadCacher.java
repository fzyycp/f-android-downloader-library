...
import cn.faury.android.library.downloader.file_download.db_recorder.DownloadFileDbHelper;
import cn.faury.android.library.downloader.file_move.DownloadFileMover;
import cn.faury.android.library.downloader.file_rename.DownloadFileRenamer;
import cn.faury.android.library.downloader.listener.OnDownloadFileChangeListener;
import cn.faury.android.library.downloader.listener.OnDownloadFileChangeListener.Type;
...
public class DownloadCacher implements DownloadRecorder, DownloadFileMover, DownloadFileDeleter, DownloadFileRenamer {
    ...
    private static final String TAG = DownloadCacher.class.getName();
...
// TODO:        ContentDbDao dao = mDownloadFileDbHelper.getContentDbDao(DownloadFileInfo.Table.TABLE_NAME_OF_DOWNLOAD_FILE);
    ...
    private void checkDownloadFileStatus(DownloadFileInfo downloadFileInfo) {
        ...
        try {
            ...
            if (FileUtils.isFilePath(saveFilePath)) {
                saveFile = new File(saveFilePath);
            }
            if (FileUtils.isFilePath(tempFilePath)) {
            ...
            if (downloadFileInfo.getStatus() == Status.DOWNLOAD_STATUS_FILE_NOT_EXIST) {
                ...
                if (saveFile != null && saveFile.length() == downloadFileInfo.getDownloadedSizeLong() &&
                    ...
                    // file completed
                    Logger.d(TAG, "checkDownloadFileStatus，文件已下载完，但当前状态为文件不存在，需要更改状态为已下载完成，url:" + downloadFileInfo
                            .getUrl());
                ...
                if (!handled) {
                    ...
                    if ((tempFile != null && tempFile.exists() && tempFile.length() > 0) || (saveFile != null &&
                        ...
                        // file error
                        Logger.d(TAG, "checkDownloadFileStatus，文件未下载完/下载文件出现问题，但当前状态为文件不存在，需要更改状态为下载出错，url:" + 
                    ...
                    if (saveFile != null) {
                        ...
                        if (!saveFile.exists()) {
                            ...
                            if (tempFile != null && tempFile.exists() && tempFile.length() == downloadFileInfo
                                ...
                                Logger.d(TAG, "checkDownloadFileStatus，文件下载完成，但是文件不存在了，需要更改状态为文件不存在，url:" + 
                                        downloadFileInfo.getUrl());
                ...
                // other status
                    ...
                    if (tempFile != null && tempFile.exists() && tempFile.length() > 0) {
                        ...
                        Logger.d(TAG, "checkDownloadFileStatus，文件没有下载完成，但是文件不存在了，需要更改状态为文件不存在，url:" + downloadFileInfo
                                .getUrl());
...
// TODO:        ContentDbDao dao = mDownloadFileDbHelper.getContentDbDao(DownloadFileInfo.Table.TABLE_NAME_OF_DOWNLOAD_FILE);
    ...
    public DownloadFileInfo getDownloadFileBySavePath(String savePath, boolean includeTempFilePath) {
        ...
        if (!FileUtils.isFilePath(savePath)) {
            return null;
        }
...
//                    .TABLE_NAME_OF_DOWNLOAD_FILE);
    ...
    public DownloadFileInfo getDownloadFile(String url) {
        ...
        if (downloadFileInfo == null && UrlUtils.isUrl(url)) {
...
//                    .TABLE_NAME_OF_DOWNLOAD_FILE);
    ...
    public List<DownloadFileInfo> getDownloadFiles() {
        ...
        if (CollectionsUtils.isEmpty(mDownloadFileInfoMap)) {
            initDownloadFileInfoMapFromDb();
        }
    ...
    public void recordStatus(String url, int status, int increaseSize) throws Exception {
        ...
        Logger.i(TAG, TAG + ".recordStatus 记录状态：status：" + status + "，increaseSize：" + increaseSize + "，url：" + url);
