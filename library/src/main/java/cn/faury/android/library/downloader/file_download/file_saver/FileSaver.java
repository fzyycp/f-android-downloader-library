package cn.faury.android.library.downloader.file_download.file_saver;
...
import cn.faury.android.library.downloader.base.FailException;
import cn.faury.android.library.downloader.base.FailReason;
...
public class FileSaver implements Save, Stoppable {
    ...
    private static final String TAG = FileSaver.class.getName();
    ...
    public void saveData(ContentLengthInputStream inputStream, long startPosInTotal) throws FileSaveException {
        ...
        try {
            ...
            FileUtils.createFileParentDir(mTempFilePath);
            FileUtils.createFileParentDir(mSaveFilePath);
            ...
            Logger.d(TAG, TAG + ".saveData 1、准备写文件缓存，路径：" + tempFile.getAbsolutePath() + "，url：" + url);
                ...
                switch (mDownloadNoticeStrategy) {
                    case NOTICE_AUTO:
                        ...
                        if (dTime >= DownloadNoticeStrategy.NOTICE_BY_TIME.getValue()) {
                            // 2.saving
                            Logger.d(TAG, TAG + ".saveData 2、正在写文件缓存，已处理：" + handledFileSize + "，总共需要处理：" +
                                    needHandleSize + "，完成（百分比）：" + (StringUtils.formatNumberPercent(((double) handledFileSize /
                            ...
                            if (needNotifySize >= maxNotifySize) {
                                // 2.saving
                                Logger.d(TAG, TAG + ".saveData 2、正在写文件缓存，已处理：" + handledFileSize + "，总共需要处理：" +
                                        needHandleSize + "，完成（百分比）：" + (StringUtils.formatNumberPercent(((double) 
            ...
            if (needNotifySize > 0) {
                // 2、saving
                Logger.d(TAG, TAG + ".saveData 2、正在写文件缓存，已处理：" + handledFileSize + "，总共需要处理：" + needHandleSize +
                        "，完成（百分比）：" + (StringUtils.formatNumberPercent(((double) handledFileSize / needHandleSize) * 100)) +
                        "%，url：" + url);
            ...
            if (needHandleSize == handledFileSize && tempFile.length() == mFileTotalSize) {
                ...
                if (!isCompleted) {
                    ...
                    isCompleted = FileUtils.copyFile(tempFile, saveFile, true);
                }
                // failed
                if (!isCompleted) {
                ...
                Logger.d(TAG, TAG + ".saveData 3、文件保存完成，路径：" + saveFile.getAbsolutePath() + "，url：" + url);
            }
            ...
            Logger.d(TAG, TAG + ".saveData 3、文件保存【已结束】，是否有异常：" + hasException + "，保存路径：" + filePath + "，url：" + url);
        }
    }
    ...
    private void notifyStart() {
        ...
        Logger.i(TAG, "file-downloader-save 准备开始保存数据");
    }
    ...
    private boolean notifySaving(int needNotifySize, long needHandleSize) {
        if (mOnFileSaveListener != null) {
            ...
            Logger.i(TAG, "file-downloader-save 正在保存数据，needNotifySize：" + needNotifySize + "，needHandleSize：" +
                    needHandleSize);
    ...
    private void notifyEnd(int increaseSize, boolean complete) {
        ...
        Logger.i(TAG, "file-downloader-save 保存数据完成，是否整个文件全部下载完成：" + complete);
    }
    ...
    private void checkIsStop() throws FileSaveException {
        ...
        if (isStopped()) {
            ...
            Logger.e(TAG, TAG + ".checkIsStop --已经处理完了/强制停止了，不能再处理数据！");
