...
import cn.faury.android.library.downloader.listener.OnDeleteDownloadFileListener.DeleteDownloadFileFailReason;
import cn.faury.android.library.downloader.listener.OnDeleteDownloadFileListener.OnDeleteDownloadFileFailReason;
...
public class DownloadDeleteManager {
    ...
    private static final String TAG = DownloadDeleteManager.class.getName();
    ...
    public void delete(final String url, final boolean deleteDownloadedFileInPath, final OnDeleteDownloadFileListener
        ...
        if (!mDownloadTaskPauseable.isDownloading(url)) {
            ...
            Logger.d(TAG, TAG + ".delete 下载任务已经暂停，可以直接删除，url:" + url);

            singleDeleteInternal(url, deleteDownloadedFileInPath, onDeleteDownloadFileListener);
        } else {

            Logger.d(TAG, TAG + ".delete 需要先暂停下载任务后删除,url:" + url);
            ...
            mDownloadTaskPauseable.pause(url, new OnStopFileDownloadTaskListener() {
                ...
                public void onStopFileDownloadTaskSucceed(String url) {
                    ...
                    Logger.d(TAG, TAG + ".delete 暂停下载任务成功，开始删除，url:" + finalUrl);
                ...
                public void onStopFileDownloadTaskFailed(String url, StopDownloadFileTaskFailReason failReason) {
                    ...
                    Logger.d(TAG, TAG + ".delete 暂停下载任务失败，无法删除，url:" + finalUrl);
