...
class MoveDownloadFileTask implements Runnable {
    ...
    private static final String TAG = MoveDownloadFileTask.class.getName();
    ...
    public void run() {
        ...
        try {
            ...
            if (newFile != null && newFile.getParentFile() != null && !newFile.getParentFile().exists()) {
                FileUtils.createFileParentDir(newFile.getAbsolutePath());
            }
            ...
            // ------------start notifying caller------------
            {
                ...
                if (failReason == null) {
                    ...
                    Logger.d(TAG, ".run 移动成功，url：" + mUrl);
                } else {
                    // 2.move failed
                    notifyFailed(downloadFileInfo, failReason);

                    Logger.w(TAG, ".run 移动失败，url：" + mUrl + ",failReason:" + failReason.getType());
                }
            }
            // ------------end notifying caller------------

            Logger.d(TAG, ".run 文件移动任务【已结束】，是否有异常：" + (failReason == null) + "，url：" + mUrl);
        }
    }
