...
import cn.faury.android.library.downloader.DownloadConfiguration;
import cn.faury.android.library.downloader.DownloadConfiguration.Builder;
...
import cn.faury.android.library.downloader.file_download.base.Pauseable;
import cn.faury.android.library.downloader.listener.OnDetectBigUrlFileListener;
import cn.faury.android.library.downloader.listener.OnDetectBigUrlFileListener.DetectBigUrlFileFailReason;
import cn.faury.android.library.downloader.listener.OnDetectUrlFileListener;
import cn.faury.android.library.downloader.listener.OnDetectUrlFileListener.DetectUrlFileFailReason;
...
public class DownloadTaskManager implements Pauseable {
    ...
    private void addAndRunDownloadTask(final String callerUrl, DownloadFileInfo downloadFileInfo,
        ...
        downloadTask.setOnTaskRunFinishListener(new OnTaskRunFinishListener() {
            ...
            public void onTaskRunFinish() {
                ...
                synchronized (mDownloadTaskLock) {
                    ...
                    Map<Thread, StackTraceElement[]> threads = Thread.getAllStackTraces();
                    Logger.e(TAG, "mRunningDownloadTaskMap，--移除--：" + downloadTask.getUrl() + "，task：" + downloadTask
        ...
        synchronized (mDownloadTaskLock) {
            ...
            if (taskInMap != null) {
                // running, ignore
                Map<Thread, StackTraceElement[]> threads = Thread.getAllStackTraces();
                Logger.d(TAG, "mRunningDownloadTaskMap，忽略2：" + downloadTask.getUrl() + "，old task：" + taskInMap.hashCode
                        () + "，线程数：" + threads.size());
                return;
            }
            ...
            Map<Thread, StackTraceElement[]> threads = Thread.getAllStackTraces();
            Logger.d(TAG, "mRunningDownloadTaskMap，--增加--：" + downloadTask.getUrl() + "，task：" + downloadTask.hashCode()
                    + "，线程数：" + threads.size());
        }
        // exec the task
    ...
    public void createAndStart(String url, String saveDir, String fileName, DownloadConfiguration
        ...
        if (detectUrlFileInfo != null) {
            if (FileUtils.isFilePath(saveDir)) {
    ...
    private void createAndStartByDetectUrlFile(String callerUrl, DetectUrlFileInfo detectUrlFileInfo,
        ...
        if (!DownloadFileUtil.isLegal(detectUrlFileInfo)) {
            ...
            if (failReason == null) {
                if (!UrlUtils.isUrl(callerUrl) || !callerUrl.equals(detectUrlFileInfo.getUrl())) {
    ...
    private void pauseInternal(String url, final OnStopFileDownloadTaskListener onStopFileDownloadTaskListener) {
        ...
        Logger.d(TAG, "pauseInternal fileDownloadTask是否已经停止：" + (downloadTask != null ? downloadTask.isStopped() : true) +
                ",url：" + url);
        ...
        if (downloadTask != null && !downloadTask.isStopped()) {
            ...
            downloadTask.setOnStopFileDownloadTaskListener(new OnStopFileDownloadTaskListener() {
                ...
                public void onStopFileDownloadTaskSucceed(String url) {
                    ...
                    Logger.d(TAG, "pauseInternal 暂停成功，url：" + finalUrl);
                ...
                public void onStopFileDownloadTaskFailed(String url, StopDownloadFileTaskFailReason failReason) {
                    ...
                    Logger.d(TAG, "pauseInternal 暂停失败，url：" + finalUrl + ",failReason:" + failReason.getType());
            ...
            Logger.d(TAG, "pauseInternal 任务已经被暂停，url：" + url + ",failReason:" + failReason.getType());
