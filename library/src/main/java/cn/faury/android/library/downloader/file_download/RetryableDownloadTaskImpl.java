...
import cn.faury.android.library.downloader.file_download.DownloadTaskImpl.FinishState;
...
import cn.faury.android.library.downloader.file_download.base.RetryableDownloadTask;
...
class RetryableDownloadTaskImpl implements RetryableDownloadTask, OnFileDownloadStatusListener {
    ...
    private static final String TAG = RetryableDownloadTaskImpl.class.getName();
    ...
    private boolean notifyStatusRetrying() {
        try {
            ...
            if (mOnFileDownloadStatusListener instanceof OnRetryableFileDownloadStatusListener) {
                ...
                Logger.d(TAG, "file-downloader-status 记录【重试状态】成功，url：" + getUrl());
    ...
    public void run() {
        ...
        try {
            ...
            while (DownloadFileUtil.isTempFileExist(downloadFileInfo) && !mIsTaskStop && mHasRetriedTimes <
                ...
                Logger.d(TAG, ".run 正在重试，url：" + getUrl());
            ...
            Logger.d(TAG, ".run 文件下载任务【已结束】，是否有异常：" + hasException + "，url：" + getUrl());
        }
    }
