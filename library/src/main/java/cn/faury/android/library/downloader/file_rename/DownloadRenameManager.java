package cn.faury.android.library.downloader.file_rename;
...
import cn.faury.android.library.downloader.listener.OnRenameDownloadFileListener.OnRenameDownloadFileFailReason;
import cn.faury.android.library.downloader.listener.OnRenameDownloadFileListener.RenameDownloadFileFailReason;
...
public class DownloadRenameManager {
    ...
    private static final String TAG = DownloadRenameManager.class.getName();
    ...
    OnRenameDownloadFileListener onRenameDownloadFileListener) {
        ...
        if (!mDownloadTaskPauseable.isDownloading(url)) {
            ...
            Logger.d(TAG, TAG + ".rename 下载任务已经暂停，可以直接重命名，url:" + url);

            renameInternal(url, newFileName, includedSuffix, onRenameDownloadFileListener);
        } else {

            Logger.d(TAG, TAG + ".rename 需要先暂停下载任务后重命名,url:" + url);
            ...
            mDownloadTaskPauseable.pause(url, new OnStopFileDownloadTaskListener() {
                ...
                public void onStopFileDownloadTaskSucceed(String url) {
                    ...
                    Logger.d(TAG, TAG + ".rename 暂停下载任务成功，开始重命名，url:" + finalUrl);
                ...
                public void onStopFileDownloadTaskFailed(String url, StopDownloadFileTaskFailReason failReason) {
                    ...
                    Logger.d(TAG, TAG + ".rename 暂停下载任务失败，无法重命名，url:" + finalUrl);
