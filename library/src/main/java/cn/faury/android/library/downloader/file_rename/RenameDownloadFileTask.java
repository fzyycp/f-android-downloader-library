...
class RenameDownloadFileTask implements Runnable {
    ...
    private static final String TAG = RenameDownloadFileTask.class.getName();
        ...
        try {
            ...
            // ------------start notifying caller------------
            {
                ...
                if (failReason == null) {
                    ...
                    Logger.d(TAG, TAG + ".run 重命名成功，url：" + mUrl);
                } else {
                    // 2.rename failed
                    notifyFailed(downloadFileInfo, failReason);

                    Logger.d(TAG, TAG + ".run 重命名失败，url：" + mUrl + ",failReason:" + failReason.getType());
                }
            }
            // ------------end notifying caller------------

            Logger.d(TAG, TAG + ".run 重命名任务【已结束】，是否有异常：" + (failReason == null) + "，url：" + mUrl);
        }
    }
