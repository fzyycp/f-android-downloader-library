...
import cn.faury.android.library.downloader.base.BaseDownloadConfigBuilder;
...
public class FileDownloadConfiguration {
    ...
    public static class Builder extends BaseDownloadConfigBuilder {
        ...
        public Builder(Context context) {
            ...
            this.mFileDownloadDir = StorageUtils.getStoragePackageDir(mContext) + File.separator + FCommonGlobalConfigure.DIR_DOWNLOAD;
        ...
        public Builder configFileDownloadDir(String fileDownloadDir) {
            if (!TextUtils.isEmpty(fileDownloadDir)) {
                ...
                if (!file.exists()) {
                    Logger.v(TAG, "configFileDownloadDir 要设置的文件下载保存目录：" + fileDownloadDir + " 还不存在，需要创建！");
                    if (file.mkdirs()) {
                        this.mFileDownloadDir = fileDownloadDir;
                        Logger.v(TAG, "configFileDownloadDir 要设置的文件下载保存目录：" + fileDownloadDir + " 创建成功！");
                    } else {
                        Logger.w(TAG, "configFileDownloadDir 要设置的文件下载保存目录：" + fileDownloadDir + " 创建失败！");
                    }
                } else {
                    this.mFileDownloadDir = fileDownloadDir;
                    Logger.i(TAG, "configFileDownloadDir 要设置的文件下载保存目录：" + fileDownloadDir + " 已存在，不需要创建！");
                }
            }
            Logger.d(TAG, "configFileDownloadDir 最终文件下载保存目录：" + this.mFileDownloadDir);
            return this;
        }
        ...
        public Builder configDownloadTaskSize(int downloadTaskSize) {
            if (downloadTaskSize >= 1 && downloadTaskSize <= MAX_DOWNLOAD_TASK_SIZE) {
                ...
                Logger.i(TAG, "configDownloadTaskSize 配置同时下载任务的数量失败，downloadTaskSize：" + downloadTaskSize);
            }
            return this;
        }
    ...
    private static final String TAG = FileDownloadConfiguration.class.getName();
