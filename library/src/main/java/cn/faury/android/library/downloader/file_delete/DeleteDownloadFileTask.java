...
class DeleteDownloadFileTask implements Runnable {
    ...
    private static final String TAG = DeleteDownloadFileTask.class.getName();
    ...
    public void run() {
        ...
        try {
            ...
            Logger.d(TAG, TAG + ".run 数据库删除成功url：" + mUrl);
            ...
            Logger.d(TAG, TAG + ".run 文件删除成功url：" + mUrl);
        } catch (Exception e) {
            e.printStackTrace();
            ...
            // ------------start notifying caller------------
            {
                ...
                if (failReason == null) {
                    ...
                    Logger.d(TAG, TAG + ".run 删除成功，url：" + mUrl);
                } else {
                    // 2.delete failed
                    notifyFailed(downloadFileInfo, failReason);

                    Logger.d(TAG, TAG + ".run 删除失败，url：" + mUrl + "，failReason:" + failReason.getType());
                }
            }
            // ------------end notifying caller------------

            Logger.d(TAG, TAG + ".run 文件删除任务【已结束】，是否有异常：" + (failReason == null) + "，url：" + mUrl);
        }
    }
